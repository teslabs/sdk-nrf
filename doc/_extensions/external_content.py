...
def adjust_includes(fname: Path, basepath: Path, directives: List[str]) -> None:
    ...
    def _adjust(m):
        directive = m.group(1)
        fpath = Path(m.group(2))

        # ignore absolute paths or existing files
        if str(fpath).startswith("/") or (fname.parent / fpath).exists():
            fpath_adj = fpath
        else:
            fpath_adj = os.path.relpath(basepath / fpath, fname.parent)
    ...
    with open(fname, "r+") as f:
        ...
        content_adj, modified = re.subn(
            r"\.\. (" + "|".join(directives) + r")::\s*(.*)", _adjust, content
        )
        if modified:
            f.seek(0)
            f.write(content_adj)
